...
const BiddingDisplayStudy = () => {
  ...
  const biddingData = [
    ...
    },
    {
      ...
      price: 28000,
      services: [
        '院校规划：综合排名Top50学校申请方案（8所学校），专业匹配度分析+录取概率评估',
        '申请材料：完整申请材料包装，个人陈述+推荐信+CV专业指导，突出个人特色',
        '语言提升：托福/雅思/GRE考试规划+模拟练习，提供在线课程+1对1辅导',
        '后续服务：录取后选校指导+I-20申请+行前准备+宿舍申请+接机安排'
      ]
    },
    {
      id: 3,
      ...
      price: 22000,
      services: [
        '学校申请：中等排名院校申请包（6所学校），注重专业匹配和录取率保障',
        '文书修改：申请文书修改润色服务，语法检查+逻辑优化+格式调整',
        '考试指导：标准化考试备考计划，提供备考资料+在线模拟考试系统',
        '签证咨询：签证材料清单+填表指导+面试预约，提供详细签证攻略'
      ]
    },
    {
      id: 4,
      ...
      price: 45000,
      services: [
        '顶级服务：常春藤+Top10名校冲刺包（12所学校），前哈佛招生官亲自指导',
        '全程陪伴：从申请到入学全程1对1专属顾问，24小时随时响应各类问题',
        '背景包装：顶级实习推荐+知名导师科研+国际期刊论文发表+竞赛指导',
        'VIP服务：绿色通道签证+豪华接机+名校参观+校友资源对接+就业指导'
      ]
    }
  ];
          ...
          <div className="flex justify-between items-center p-4">
            <div className="flex items-start">
              <div className="pixel-border w-16 h-16 flex items-center justify-center border-2 border-black mr-4">
                <FontAwesomeIcon icon={taskInfo.icon} className="text-2xl text-blue-600" />
              </div>
              <div>
            ...
            >
              ...
              <div className="border-r-2 border-black flex flex-col justify-center items-center w-48 p-4 flex-shrink-0">
                <div className="pixel-border w-20 h-20 flex items-center justify-center border-2 border-black mb-3">
                  <FontAwesomeIcon icon={bid.icon} className="text-3xl text-blue-600" />
                </div>
              ...
              <div className="border-l-2 border-black flex items-center w-36 p-4 flex-shrink-0">
                <button
                  ...
                  className={`pixel-button w-full h-12 text-sm font-bold uppercase transition-all duration-200 ${
                    selectedBid === bid.id ? 'bg-blue-600 border-blue-600 text-white' : 'hover:bg-gray-100'
                  }`}
                >
...
export default BiddingDisplayStudy; 